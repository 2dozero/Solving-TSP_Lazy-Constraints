...
# # 사용 예
# directory_path = './image/berlin52/' # 현재 디렉토리를 사용
# output_filename = './gif/berlin52_output.gif'
...
def extract_number(path):
    file_name = os.path.basename(path)  # 전체 경로에서 파일 이름만 추출
    match = re.search(r'\d+', file_name)
    if match:
        return int(match.group())
    return 0
...
# 사용 예
directory_path = './image/st70/' # 현재 디렉토리를 사용
output_filename = './gif/st70_output.gif'
duration = 0.5 # 각 이미지가 0.5초 동안 보여짐
create_gif_from_pngs(directory_path, output_filename, duration)
